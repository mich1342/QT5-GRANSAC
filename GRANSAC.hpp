...
namespace GRANSAC
{
	...
	public:
		...
		virtual ~RANSAC(void) {};
        void setMaxThreads(int a){
            if (a < 1){
                max_allowable_thread = std::max(1, omp_get_max_threads());
            }else{
                max_allowable_thread = a;
            }
            std::cout << "[ INFO ]: Maximum allowable threads: " << max_allowable_thread << std::endl;
        }
		void Reset(void)
		{
		...
		bool Estimate(const std::vector<std::shared_ptr<AbstractParameter>> &Data)
		{
            ...
            omp_set_num_threads(max_allowable_thread);
